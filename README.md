...
| 블로그 | 주소 | 성격 | 글 수 |
| --- | --- | --- | --- |
| 주식 한 입 | https://ericreview.tistory.com | 주식 투자 입문 (초보 교육) | 104 |
...
- **"제일 급한 게 뭔가?"** → 구글 서치콘솔은 **이미 등록·사이트맵 제출 완료로 실측 확인**(8/18)됐습니다. **리디렉션 오류 12편은 8/26에 대응 완료**했습니다(원인: Googlebot 스마트폰의 `/m/` 모바일 경로). 색인 안 된 164개도 8/27에 분해했습니다 — **3분의 2가 `/m/` 모바일 URL·카테고리 페이지**로 정상이고, 실제 대상은 데스크톱 글 33편입니다(9편 요청 완료). **원고 11편 전부 발행·공개 완료 — 비공개 글 0편.** 8/29 413번(vLLM Structured Outputs) 발행으로 hadesyi 링크 부채 청산. 남은 병목은 ① **신규 4편 + 데스크톱 24편 색인 요청**, ② 네이버 서치어드바이저 등록입니다. **공개 전환 시 발행일 갱신을 반드시 함께** 하십시오(원칙 6). 진단 배경은 [`docs/traffic-and-seo.md`](docs/traffic-and-seo.md), 실측 수치는 워크로그 참고.
